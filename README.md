...
| **What-if analysis** | One screen, no scrolling. A CSV upload bar sits on top: drop or browse a file and get a batch summary, row problems by line and field, and a compact list of scored rows. Click a row to see its result, add the batch to consignments, or download the results. Below it, a single-consignment check asks only the inputs the CSV needs plus route, mode and cargo value. It shows the 7-day risk, expected loss, the top three risk drivers and the best option. |
